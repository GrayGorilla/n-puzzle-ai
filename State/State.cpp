...
shared_ptr<State> State::copyAndSwap(const pair<int, int> a, const pair<int, int> b, const std::shared_ptr<State> currState) const {
    ...
    return make_shared<State>(child, this->distance + 1, currState);
}
...
// Factory
shared_ptr<State> State::makeMove(const Direction dir, const shared_ptr<State> currState) const {
    ...
    switch (dir) {
        case UP:
            if (blankSpace.first > 0) {
                ...
                return copyAndSwap(blankSpace, destination, currState);
            } else {
                return nullptr;
            }
        case DOWN:
